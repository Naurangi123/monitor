...
function fmtRSS(bytes) {
  return bytes ? (bytes / 1024 / 1024).toFixed(1) : "0.0";
}
...
function renderRows() {
  ...
  currentData.forEach((p) => {
    ...
    tr.innerHTML = `
      ...
      <td>
        ${p.name || '<span class="muted">(unknown)</span>'}
        ${
          hasCmdline
          ...
          <table style="width:50%; border-collapse:collapse; font-size:13px;">
            ...
            <tbody>
              ${exeArgs
                .map((arg, idx) => {
                  const truncated =
                    arg.length > 250 ? arg.slice(0, 250) + "…" : arg;
                  return `
                    <tr>
                      <td style="padding:6px; border:1px solid #d7dee9;">${idx + 1}</td>
...
async function refresh() {
  ...
  currentData = Array.isArray(data.processes) ? [...data.processes] : [];
  tsEl.textContent = `Snapshot: ${new Date(
    data.taken_at
  ).toLocaleString()}`;
